...
mod tests {
    ...
    fn can_convert_to_seconds_from_epoch_and_back() {
        ...
        let deserialized: PlabbleDateTime = BinaryDeserializer::<()>::from_bytes(&bytes, None).unwrap();
    ...
    fn plabble_datetime_max_value() {
        ...
        let max_date: PlabbleDateTime = BinaryDeserializer::<()>::from_bytes(&max_bytes, None).unwrap();
