...
use crate::packets::{base::PlabblePacketBase, body::session::{SessionRequestBody, SessionResponseBody}, header::{request_header::PlabbleRequestHeader, response_header::PlabbleResponseHeader}};
...
pub trait SerializableRequestBody {
    fn to_bytes(&self, context: &mut RequestSerializationContext) -> Result<Vec<u8>, SerializationError>;
    fn from_bytes(bytes: &[u8], context: &mut RequestSerializationContext) -> Result<Self, DeserializationError> where Self: Sized;
}

pub trait SerializableResponseBody {
    fn to_bytes(&self, context: &mut ResponseSerializationContext) -> Result<Vec<u8>, SerializationError>;
